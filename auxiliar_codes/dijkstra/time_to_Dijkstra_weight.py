...
def solve_map(robot: Robot, maze: Maze) -> None:
    ...
    from types_and_constants import PI, TILE_SIZE

    times = []
    for i in range(10):
        start = time()
        robot.run(Rotate("right" if i % 2 == 0 else "left", PI / 2, maze=maze))
        end = time()

        print(f"Demorou rodando: {end - start}s")
        times.append(end - start)

    print(f"Rodando demora na média: {sum(times) / len(times)}s")
    print(times)
    ...
    for i in range(10):
        start = time()
        robot.run(
            Move("forward" if i % 2 == 0 else "backward", dist=TILE_SIZE, maze=maze)
        )
        end = time()

        print(f"Demorou andando: {end - start}s")
        times.append(end - start)

    print(f"Andando demora na média: {sum(times) / len(times)}s")
    print(times)
