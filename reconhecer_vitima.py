    ...
    if parede_esquerda():
        ...
        if np.any(mask_yellow > 0):
            ...
            if polygon_count <= 20:
                ...
                delay(1300)
                codificar_tipo("S")  
        else: print(f'nenhuma vitima encontrada!, media {media}')  
    else: print('nenhuma vítima encontrada!')

# Main loop
while robot.step(timeStep) != -1:
    if cameraE is not None:
        count_and_display_polygons()
    else:
