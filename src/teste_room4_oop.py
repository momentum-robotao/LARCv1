try:
    ...
    def conferir_circulo(centro: list, ponto: list, raio: float) -> bool:
        ...
        """
        if (math.pow(ponto[0] - centro[0], 2) + math.pow(ponto[1] - centro[1], 2)) < (
            math.pow(raio, 2)
        ):
            return True
        else:
        ...
        try:
            ...
            def get_mapa():
                ...
                for i in range(360):
                    ...
                    if (get_distance(i) > (1.5) * 0.12) or (get_distance(i) < 0.005):
                        ...
                        theta = soma + i
                        x.append(
                            get_distance(i) * math.cos(radians(theta))
                            + get_posicao_atual()[0]
                        )
                        y.append(
                            get_distance(i) * math.sin(radians(theta))
                            + get_posicao_atual()[1]
                        )
            ...
            def parede_tras():
                ...
                for i in range(100, 260):
                    if get_distance(i) < menor_valor:
                        menor_valor = get_distance(i)
                if menor_valor < desejada:
                    return menor_valor
            ...
            def virar_max():
                ...
                for i in range(359, 285, -1):
                    if get_distance(i) <= desejada + 0.01:
                        continue
                    else:
                        angle = 360 - i
                return angle
            ...
            def color_sala4():
                ...
                if color_sensor.check_colored_special_tile() in [
            ...
            def ver_obstaculo():
                ...
                tras_esquerda = distance_sensor.pegar_distancia(
                    distance_sensor._backleft
                )
                tras_direita = distance_sensor.pegar_distancia(
                    distance_sensor._backright
                )
                distancia = 0.037
                condicao = False
                onde = []
            ...
            def sala4():
                ...
                if color_sala4():
                    ...
                    if not tem_parede_frente() or not lidar.has_wall("front"):
                        print(
                            f"NAO TEM NADA NA MINHA FRENTE ; d[80] : {get_distance(80)})"
                        )
                        move_until()
                        condicao_sala4 = True
    ...
    if (os.getenv("DEBUG", "") + " ").upper()[0] in ["T", "1"]:
        import logging
