...
class ColorSensor(Device):
    ...
    def get_color(self) -> bytes:
        ...
        logger.info(f"Cor reconhecida: {color}", System.color_sensor_measures)
        return color
    ...
    def get_RGB_color(self) -> RGB:
        ...
        logger.info(f"Cor RGB reconhecida: {color}", System.color_sensor_measures)
        return color

    def check_colored_special_tile(self) -> ColoredSpecialTile | None:
        color = self.get_RGB_color()
        logger.info(f"Cor do chão: {color}", System.check_tile_color)

        for test_color, special_tile_type in SPECIAL_TILE_COLOR_MAPPER.items():
            if test_color == color:
                logger.info(f"É {special_tile_type}", System.check_tile_color)
                return special_tile_type
        logger.info("Não é nada", System.check_tile_color)
        return None
