...
def ajustar_distancia():
    ...
    while robot.step(timeStep) != -1 and sensoresFrente[0].getValue() > 0.076:
            encoder_antes()
            mover_para_frente(0.008)
    while robot.step(timeStep) != -1 and sensoresFrente[0].getValue() < 0.064:
            encoder_antes()
            mover_para_tras(0.008)
         
def reconhecer_vitima():
    if parede_frente(sensoresFrente):
        while robot.step(timeStep) != -1 and sensoresFrente[0].getValue() > 0.074:
            encoder_antes()
            mover_para_frente(0.05)
        while robot.step(timeStep) != -1 and sensoresFrente[0].getValue() < 0.066:
            encoder_antes()
            mover_para_tras(0.05)
        cv2.waitKey(1)
        ...
        for cnt in contours:
            ...
            '''if len(approx) >= 13 and len(approx) <= 26:
                print('H')
                delay(1300)
                codificar_tipo('H')
                
            elif len(approx) >= 27 and len(approx) <= 33:
                print('U')
                delay(1300)
                codificar_tipo('U')
            elif len(approx) >= 60 and len(approx) <= 82:
                print('S')
                delay(1300)
                codificar_tipo('S')'''
            print(len(approx))
